(function () {
  ...
  var submitFrame = document.querySelector('.waitlist-submit-frame');
  var currentStep = 0;
  ...
  submitFrame.addEventListener('load', function () { if (!isSubmitting) return; isSubmitting = false; form.reset(); city.innerHTML = '<option value="">Selecione a UF primeiro</option>'; city.disabled = true; showStep(0); success.hidden = false; });
  form.addEventListener('submit', function (event) { if (isSubmitting || !validStep()) { event.preventDefault(); return; } isSubmitting = true; form.querySelector('[type="submit"]').disabled = true; feedback.textContent = 'Enviando sua inscrição...'; });
  updateProgress();
}());
