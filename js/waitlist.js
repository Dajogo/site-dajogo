(function () {
  ...
  form.addEventListener('submit', function (event) { if (!validStep()) { event.preventDefault(); return; } window.setTimeout(function () { form.reset(); city.innerHTML = '<option value="">Selecione a UF primeiro</option>'; city.disabled = true; showStep(0); success.hidden = false; }, 1200); });
  updateProgress();
}());
